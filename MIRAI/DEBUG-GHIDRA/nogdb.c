...

{
  ...
  if (argc < 2) {
    ...
    if (iVar1 < 0) {
      ...
      main::header = (Elf32_Ehdr *)mmap((void *)0x0,8,3,1,iVar1,0);
      if (main::header == (Elf32_Ehdr *)0xffffffffffffffff) {
        perror("mmap");
        close(iVar1);
        iVar1 = 1;
      }
      else {
        puts("[*] Current header values:");
        printf("\te_shoff:%d\n\te_shnum:%d\n\te_shstrndx:%d\n",(ulong)main::header->e_shoff,
               (ulong)main::header->e_shnum,(ulong)main::header->e_shstrndx);
        main::header->e_shoff = 0xffff;
        main::header->e_shnum = 0xffff;
        main::header->e_shstrndx = 0xffff;
        puts("[*] Patched header values:");
        printf("\te_shoff:%d\n\te_shnum:%d\n\te_shstrndx:%d\n",(ulong)main::header->e_shoff,
               (ulong)main::header->e_shnum,(ulong)main::header->e_shstrndx);
        iVar2 = msync((void *)0x0,0,4);
        if (iVar2 == -1) {
          perror("msync");
          close(iVar1);
          iVar1 = 1;
